			  ...
			  "must": [
				...
				},
				{
				  "fquery": {
					"query": {
					  "query_string": {
						"query": "metric_name:(\"AuroraReplicaLag\")"
					  }
					}
				  }
				},
...
while read line; do
	...
	if  check_exp "$metric_value <= $warning" ;then
		log_msg "OK: Replica Lag = $metric_value ms"

	elif check_exp "$metric_value > $warning && $metric_value <= $critical" ;then
		log_msg "WARNING: Replica Lag = $metric_value ms"

	elif check_exp "$metric_value > $critical"  ;then
		log_msg "CRITICAL: Replica Lag = $metric_value ms"
	else 
		log_msg "UNKNOWN: Could not determine replica lag"
	fi
