			  ...
			  "must": [
				...
				},
				{
				  "fquery": {
					"query": {
					  "query_string": {
						"query": "metric_name:(\"ApproximateAgeOfOldestMessage\")"
					  }
					}
				  }
				},
...
function run_query {
	...
	curl -sm 10  "https://$host/candlestack-$today,candlestack-$yesterday/_search?timeout=3m" \
...
while read line; do
	...
	if  check_exp "$metric_value <= $warning" ;then
		log_msg "OK: Approximate Age Of Oldest Message = $metric_value"

	elif check_exp "$metric_value > $warning && $metric_value <= $critical" ;then
		log_msg "WARNING: Approximate Age Of Oldest Message = $metric_value"

	elif check_exp "$metric_value > $critical"  ;then
		log_msg "CRITICAL: Approximate Age Of Oldest Message = $metric_value"
	else 
		log_msg "UNKNOWN: Could not determine approximate age of oldest message"
	fi
